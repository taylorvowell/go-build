# Skills index — paste into the adopted CLAUDE.md

Insert this as the `## Skills — reach for the right one` section. Drop any line whose skill doesn't fit the project's stack (e.g. omit `tailwind-v4` on Tailwind v3; omit shadcn guidance if unused).
...
- **Setup:** `/init-claude` (new project) · `/adopt` (existing project).
```
